...
#include <rapidmp/parser/python.hpp>
...
int main( int argc, char *argv[] ) {
  ...
  if( args.count("help") || !args.count("input") || !args.count("repeat") )
    ...
    for( size_t count = 0; count != repeat; ++count ) {
      auto iter = serialized.data();
      const auto end = serialized.data() + serialized.size();
      rapidmp::parse_object< rapidmp::version_1_1 >( iter, end );
    }
